...
from data_helper import load_data, SentDataSet, collate_func, convert_token_id, load_test_data, collate_func_dev, \
    TestDataSet


def evaluate(epoch, dev_dataloader):
    model.eval()
    accuracy = 0  # 准确率
    for step, batch in enumerate(dev_dataloader):
        ...
        (sent_prompt1_input_ids, sent_prompt1_attention_mask, sent_prompt1_token_type_ids,
         ...
         sent_template2_input_ids, sent_template2_attention_mask, sent_template2_token_type_ids, label_id) = batch

        # 获取向量对各文本的句向量
        s1_embedding = model(prompt_input_ids=sent_prompt1_input_ids,
        ...
        s2_embedding = model(prompt_input_ids=sent_prompt2_input_ids,
...
if __name__ == '__main__':
    ...
    dev_df = load_test_data(args.dev_data_path, tokenizer)
    dev_dataset = TestDataSet(dev_df, tokenizer)
    dev_dataloader = DataLoader(dev_dataset, shuffle=False, batch_size=args.dev_batch_size,
                                collate_fn=collate_func_dev)
    ...
    for epoch in range(args.num_train_epochs):
        ...
        # output_model_file = os.path.join(args.output_dir, "{}_ckpt.bin".format(epoch))
